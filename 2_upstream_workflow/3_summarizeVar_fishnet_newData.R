...
if(cur_run == 'unconnected'){
  ...
  summarized_HYBAS = 
    readRDS(paste0(path2linkage, 'unconnected_VAR_', run_name, cur_continent_abr_UP, '.rds')) %>%
    mutate(across(natural_only:agroforestry, ~ .x/total_cells))
  
} else{
  path2linkage = paste0(proj_dir, 'Upstream/', cur_continent, '/', cur_folder_name, '/')
  summarized_HYBAS = 
    readRDS(paste0(path2linkage, 'upstream_VAR_', run_name, cur_continent_abr_UP, '_', dist_threshold, '.rds'))
}

##############################
#Create a list of variables to summarize
# Identify columns by type
var_mean <- grep("^percent", names(summarized_HYBAS), value = TRUE)
var_mode <- NA
var_count <- grep("^tree", names(summarized_HYBAS), value = TRUE)
...
out_fish_r = 
  fasterize(out_fish_v1, fishnet.r %>% raster, field = "percent_tree_total_2022") %>%
  crop(data_sub) %>%
  rast() %>%
  plot()

out_fish_r2 =
  fasterize(out_fish_v2, fishnet.r %>% raster, field = "percent_tree_total_2022") %>%
  crop(data_sub) %>%
  rast() %>%
  plot()
...
if(cur_run == 'unconnected'){
  ...
  fwrite(out_fish_df_v1, paste0(path2output, 'upstream_', run_name, cur_continent_abr_UP, '_allWS_', 
                                 dist_threshold, '.csv'))
  
  fwrite(out_fish_df_v2, paste0(path2output, 'upstream_', run_name, cur_continent_abr_UP, '_top5up_', 
                                 dist_threshold, '.csv'))
}


##########################################################
#Compare with OG pixel wise data
########################################################## 
